...
public class MakeABookingActivity extends TabActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		departureLoc.setAdapter(new PlacesAutoCompleteAdapter(this,
				R.layout.item_list));
		destinationLoc.setAdapter(new PlacesAutoCompleteAdapter(this,
				R.layout.item_list));
		departureLocReturn.setAdapter(new PlacesAutoCompleteAdapter(this,
				R.layout.item_list));// optional
		destinationLocReturn.setAdapter(new PlacesAutoCompleteAdapter(this,
				R.layout.item_list));// optional
		...
		message = getIntent().getStringExtra(HomeActivity.EXTRA_MESSAGE);
		setupActionBar();
		setupTab();
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		switch (item.getItemId()) {
		 	case R.id.action_help:
		 		Intent intent = new Intent(MakeABookingActivity.this, HelpActivity.class);
	...
	private void addLocation(Button stopover) {
		...
		stopover.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View view) {
				...
				row.setLayoutParams(new TableRow.LayoutParams(0,
						android.widget.TableRow.LayoutParams.WRAP_CONTENT, 10f));

				Drawable remove = getResources().getDrawable(
						R.drawable.ic_action_remove);

				Button deleteButton = new Button(getApplicationContext(), null,
						android.R.attr.buttonStyleSmall);
				deleteButton.setBackgroundDrawable(remove);

				AutoCompleteTextView oneMoreLoc = new AutoCompleteTextView(
						getApplicationContext());
				oneMoreLoc.setLayoutParams(new TableRow.LayoutParams(0,
						android.widget.TableRow.LayoutParams.WRAP_CONTENT, 9f));
				...
				oneMoreLoc.setAdapter(new PlacesAutoCompleteAdapter(
						MakeABookingActivity.this, R.layout.item_list));

				// set the specific place for stop-over
				if (id == R.id.addLoc) {
					row.addView(oneMoreLoc);
					row.addView(deleteButton);
					tLayout.addView(row, new TableLayout.LayoutParams(
							LayoutParams.MATCH_PARENT,
							LayoutParams.WRAP_CONTENT));
				} else {
					row.addView(oneMoreLoc);
					row.addView(deleteButton);
					tLayoutReturn.addView(row, new TableLayout.LayoutParams(
							LayoutParams.MATCH_PARENT,
							LayoutParams.WRAP_CONTENT));
				}
		...
		editText.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View view) {
				...
				if (!editText.getText().toString().equals("")) {
					int year = Integer.parseInt(editText.getText().toString().substring(0, 4));
					int month = Integer.parseInt(editText.getText().toString().substring(5, 7));
					int day = Integer.parseInt(editText.getText().toString().substring(8,10 ));
//					Log.v("date", year + " - " + month + " - " + day);
					...
					.setPositiveButton("OK", new DialogInterface.OnClickListener() {
						public void onClick(DialogInterface dialog, int id) {
		    				...
		    				editText.setText(new StringBuilder().append(year)
		    						...
		    						.append(checkDigit(day)).append("  ")
		    						.append(checkDigit(hour)).append(":").append(checkDigit(min)));
						}
					})
		...
		submitButton.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View view) {
				...
				if (verifyFields() == true) {
					confirmView = layoutInflater.inflate(R.layout.confirmation,
							null);
	...
	private boolean verifyFields() {
		...
		View focusView = null;
		// need to add one more validation: if checkbox is ticked, convertedName
		// = sessionUser
		...
		convertedDestinationLocReturn = destinationLocReturn.getText()
				.toString();// optional
		...
		if (TextUtils.isEmpty(convertedPhone)) {
			...
			// validating that the phone number has a total of 8 numbers and it
			// begins with 6,8 or 9
		...
		if (noOfTableRow != 0) {
			loop: for (int i = 0; i < noOfTableRow; i++) {
				TableRow row = (TableRow) tLayout.getChildAt(i);
				AutoCompleteTextView tv = (AutoCompleteTextView) row
						.getChildAt(0);
			...
			for (int i = 0; i < noOfTableRow; i++) {
				...
				AutoCompleteTextView tv = (AutoCompleteTextView) row
						.getChildAt(0);
				convertedStopovers = convertedStopovers
						+ tv.getText().toString() + "; ";
			}
		}
		...
		if (switchTimeLocationBased.isChecked() == true) {
			...
			if (noOfTableRowReturn != 0) {
				loop: for (int i = 0; i < noOfTableRowReturn; i++) {
				...
				for (int i = 0; i < noOfTableRowReturn; i++) {
					...
					AutoCompleteTextView tv = (AutoCompleteTextView) row
							.getChildAt(0);
					convertedStopoversReturn = convertedStopoversReturn
							+ tv.getText().toString() + "; ";
				}
			}
		}
	...
	public void showConfirmation() {
		...
		TextView confirmedPassenger = (TextView) confirmView
				.findViewById(R.id.noPassengerValue);
		TextView confirmedDepartLoc = (TextView) confirmView
				.findViewById(R.id.departLocValue);
		TextView confirmedStopoverList = (TextView) confirmView
				.findViewById(R.id.confirmedStopoverList);// optional
		TextView confirmedDestLoc = (TextView) confirmView
				.findViewById(R.id.destLocValue);
		TextView confirmDepartLocReturn = (TextView) confirmView
				.findViewById(R.id.departLocValueReturn);// optional
		TextView confirmedStopoverListReturn = (TextView) confirmView
				.findViewById(R.id.confirmedStopoverListReturn);// optional
		TextView confirmDestLocReturn = (TextView) confirmView
				.findViewById(R.id.destLocValueReturn);// optional
		TextView confirmedStartDT = (TextView) confirmView
				.findViewById(R.id.confirmedStartDT);
		TextView confirmedStartDTReturn = (TextView) confirmView
				.findViewById(R.id.confirmedStartDTReturn);// optional
		TextView confirmedName = (TextView) confirmView
				.findViewById(R.id.confirmedName);
		TextView confirmedNumber = (TextView) confirmView
				.findViewById(R.id.confirmedNumber);
		TextView confirmedEmail = (TextView) confirmView
				.findViewById(R.id.confirmedEmail);
		TextView confirmedComments = (TextView) confirmView
				.findViewById(R.id.confirmedComments);// optional

		TextView confirmedCommentText = (TextView) confirmView
				.findViewById(R.id.comments);// optional
		TextView textStopoverList = (TextView) confirmView
				.findViewById(R.id.stopoverList);// optional text
		TextView textdepartLocReturn = (TextView) confirmView
				.findViewById(R.id.departLocReturn);// optional text
		TextView textStopoverListReturn = (TextView) confirmView
				.findViewById(R.id.stopoverListReturn);// optional text
		TextView textDestLocReturn = (TextView) confirmView
				.findViewById(R.id.destLocReturn);// optional text
		TextView textStartDTReturn = (TextView) confirmView
				.findViewById(R.id.startDTReturn);// optional text
